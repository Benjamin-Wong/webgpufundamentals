...
本文将带你了解 WebGPU 的基础知识。

<div class="warn">
阅读本文前，你需要已经掌握 JavaScript。本文会大量使用
...
<a href="https://developer.mozilla.org/zh_CN/docs/Web/JavaScript/Reference/Statements/async_function">async/await</a>,
<a href="https://developer.mozilla.org/zh_CN/docs/Web/JavaScript/Guide/Modules">ES6 模块</a>等概念。如果你还不了解 JavaScript，可以参考
<a href="https://javascript.info/">JavaScript.info</a>、<a href="https://eloquentjavascript.net/">Eloquent JavaScript</a>
或 <a href="https://www.codecademy.com/learn/introduction-to-javascript">CodeCademy</a>。
</div>

<div class="warn">如果你已经了解 WebGL，<a href="webgpu-from-webgl.html">请阅读此文</a>。</div>

WebGPU 是一个 API，它让你可以做两件基本的事情：
...
就是这样！
之后有关 WebGPU 的一切都取决于你自己。这就像学习 JavaScript、Rust 或 C++ 等编程语言一样——先学基础，然后创造性地运用这些基础来解决问题。

WebGPU 是一个极其底层的 API。 虽然你可以用它做一些小的示例，但对于很多应用来说，可能需要大量代码和认真的数据组织。 举例来说，支持 WebGPU 的 [three.js](https://threejs.org) 包含约 600k 的 JavaScript，这还只是它的基础库。这还不包括加载器、控件、后处理和许多其他功能。 同样的, [带有 WebGPU 后端的 TensorFlow](https://github.com/tensorflow/tfjs/tree/master/tfjs-backend-webgpu)也需要大约 500K 的 JavaScript 压缩包。

重点是，如果你只想在屏幕上显示点东西，选择一个现成的库会比自己从头写要好得多。

另一方面，也许你有特殊需求，也许你想修改某个现有库，也许你只是好奇它到底是怎么工作的。如果是这样，请继续阅读！

# 起步

很难决定从哪里开始。从某种程度上说，WebGPU 是一个非常简单的系统。它只做一件事：在 GPU 上运行 3 种函数——顶点着色器（Vertex Shader）、片段着色器（Fragment Shader）和计算着色器（Compute Shader）。

顶点着色器负责计算顶点位置。每当顶点着色器返回 3 个顶点位置，就会在这 3 个位置之间绘制一个三角形。 [^primitives]
...
片段着色器负责计算颜色。[^fragment-output] 在绘制三角形时，GPU 会为每个待绘制的像素调用片段着色器，片段着色器返回一个颜色值。

[^fragment-output]: 片段着色器间接将数据写入纹理。这些数据不一定是颜色。例如，输出像素所代表表面的方向向量是一种很常见的场景。

计算着色器则更加通用。它实际上只是一个函数，你告诉 GPU "执行这个函数 N 次"。GPU 每次调用你的函数时都会传入当前的迭代编号，因此你可以利用这个编号在每次迭代中做不同的事情。
...
在 GPU 上运行的函数和 JavaScript 函数一样，都只是函数而已。不同之处在于它们运行在 GPU 上，因此你需要以缓冲区（buffer）和纹理（texture）的形式将数据复制到 GPU 上，着色器函数也只能输出到这些缓冲区和纹理。你需要在着色器中指定函数将从哪些绑定（binding）或位置（location）查找数据。然后在 JavaScript 端，将持有数据的缓冲区和纹理绑定到对应的绑定或位置上。完成这些设置后，你就可以告诉 GPU 执行函数了。
...
-   **管线（Pipeline）**：它包含 GPU 将运行的顶点着色器和片段着色器。你也可以创建包含计算着色器的管线。

-   着色器通过**绑定组（Bind Groups）** 间接引用资源（缓冲区 buffer、纹理 texture、采样器 sampler）

-   管线定义了通过内部状态间接引用缓冲区的属性（Attributes）

-   属性（Attributes）从缓冲区中提取数据，并将数据输入顶点着色器

-   顶点着色器可以将数据传递给片段着色器

-   片段着色器通过渲染通道描述（render pass description）间接写入纹理

要在 GPU 上执行着色器，需要创建所有这些资源并设置状态。创建资源相对简单。一个值得注意的点是，大多数 WebGPU 资源在创建后都无法更改。你可以更改它们的内容，但无法更改它们的大小、用途、格式等。如果需要更改这些属性，就必须创建一个新资源并销毁旧资源。

有些状态是通过创建和执行命令缓冲区来设置的。命令缓冲区正如其名——它就是一个存放命令的缓冲区。你创建命令编码器（Command Encoder），编码器将命令编码到命令缓冲区中。编码器*完成*（_finish_）编码后，会返回它创建的命令缓冲区。然后你就可以*提交*（_submit_）该命令缓冲区，让 WebGPU 执行这些命令。
...
现在我们已经创建了着色器模块，接下来需要创建一个渲染管线（Render Pipeline）：
...
这里没什么复杂的。我们将 `layout` 设置为 `'auto'`，意思是让 WebGPU 从着色器中自动推断数据的布局。不过我们目前并没有使用任何数据。

然后，我们告诉渲染流水线在使用顶点着色器时使用着色器模块中的 `vs` 函数，在使用片段着色器时使用 `fs` 函数。除此之外，我们还要告诉它第一个渲染目标的格式。`渲染目标"（render target）是指我们最终要渲染到的纹理。创建管线时，必须指定该管线最终要渲染到的纹理格式。
...
现在，我们已经看到了一个非常小的 WebGPU 工作示例。显而易见，在着色器中硬编码三角形并不灵活。我们需要一些提供数据的方法，我们将在接下来的文章中介绍这些方法。从上面的代码中可以总结出以下要点：

-   WebGPU 只是运行着色器。你可以在其中填充代码，做一些有用的事情
-   着色器在着色器模块中指定，然后通过管线使用
-   WebGPU 可以绘制三角形
-   WebGPU 绘制到纹理上（我们恰好从画布获取了纹理）
-   WebGPU 的工作方式是编码命令，然后提交命令
...
现在我们已经创建了着色器，需要创建一个计算管线（Compute Pipeline）：
...
[^layout-auto]: `layout: 'auto'`很方便，但使用 `layout: 'auto'` 布局无法在不同管线之间共享绑定组。本站大多数示例都不会在多个管线中共用绑定组。我们将在另一篇文章中介绍[显式布局](webgpu-bind-group-layouts.html)。︎
...
可以在 JavaScript 中映射的 WebGPU 缓冲区不能用于其他太多用途。换句话说，我们无法映射刚刚创建的缓冲区，如果我们尝试添加标记使其可以映射，就会得到一个与使用 `STORAGE` 不兼容的错误信息。
...
我们从管线中获取绑定组的布局。然后设置 bindGroup 的`entries`。`pipeline.getBindGroupLayout(0)` 中的 0 对应着着色器中的 `@group(0)`。`{binding：0 ...` `entry`对应着着色器中的 `@group(0) @binding(0)`。
